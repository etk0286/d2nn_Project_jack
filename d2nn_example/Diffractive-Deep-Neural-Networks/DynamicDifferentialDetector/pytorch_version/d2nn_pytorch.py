...
class DynamicDifferentialDetector(nn.Module):
    def __init__(self, size: int = SIZE, units: int = 10, temperature: float = TEMPERATURE):
        super().__init__()
        self.size = size
        self.units = units
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return I_diff / self.temperature
...
class D2NNModel(nn.Module):
    def __init__(self, size: int = SIZE):
        ...
        self.detector = DynamicDifferentialDetector(size=size)
